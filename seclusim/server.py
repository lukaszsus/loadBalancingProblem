class Server:
    ...
    def decrement_active_conn(self):
        for i in range(len(self._active_connections_times_to_close)):
            self._active_connections_times_to_close[i] -= 1
    ...
    def get_mean_load(self):
        return sum(self._connections_number_history)/len(self._connections_number_history)/self._weight
